function convert(input) {
    ...
    for (var k = 0; k < res.length; k++) {
      ...
      if (div.length === 3) {
        ...
        var dividend = input.substring(div[0], div[2] - 1);
        var divisor = input.substring(div[2] + 2, div[1] + 1);
        
        if (!isNumDivisor(divisor)) {
          resText.push({
            'division': division,
            'dividend': dividend,
            'divisor': divisor
    ...
    getOutput(sortArrayByObjPropLength(resUnique.reverse(), 'division', 'desc'));
  }
...
function getOutput(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var dividend = data[i].dividend;
        var divisor = data[i].divisor;
        var divisionNew = wrapperfunction + '(' + dividend + ', ' + divisor + ')';
...
function isNumDivisor(divisor) {
  var regexNum = /[\d\. \*\/\+\-\(\)]+/;
  
  for (var i = 0; i < divisor.length; i++) {
    if (!regexNum.test(divisor[i])) {
      return false;
    }
  }
